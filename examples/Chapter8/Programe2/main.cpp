...
ImportedModel pyramid("Assets/Models/Chapter8/pyr.obj");
...
void setupShadowBuffers(GLFWwindow* window) {
	...
	//SOIL2中封装了 glGenTextures和glTexImage2D
	...
	//将纹理换行模式改为夹紧到边缘 纹理之外的值会被限制为边缘处的值（不重复）
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
}

void resizeShadowBuffers(GLFWwindow* window) {
	glfwGetFramebufferSize(window, &width, &height);
	scSizeX = width;
	scSizeY = height;
...
void init(GLFWwindow* window) {
	renderingProgram1 = Utils::createShaderProgram(
			"Shader/Chapter8/ShadowVert.glsl",
			"Shader/Chapter8/ShadowFrag.glsl");
	renderingProgram2 = Utils::createShaderProgram(
			"Shader/Chapter8/vertShader.glsl",
			"Shader/Chapter8/PCFfragShader.glsl");
...
void window_size_callback(GLFWwindow* win, int newWidth, int newHeight) {
	...
	resizeShadowBuffers(win); //Fix 窗口变动阴影渲染修改
}
